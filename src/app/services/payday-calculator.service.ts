...
export interface PaydayInfo {
  ...
  adjustedDailyBudget: number; // Budget quotidien ajusté (remaining / daysUntilPayday)
  realisticDailyBudget: number; // Budget quotidien réaliste (revenus - dépenses réalisées) / jours restants
...
export class PaydayCalculatorService {
  ...
  calculatePaydayInfo(
    ...
    realisticRemainingBudget?: number
  ): PaydayInfo {
    ...
    const daysUntilPayday = Math.ceil(
      (nextPayday.getTime() - today.getTime()) / millisecondsPerDay
    );
    const daysSincePayday = Math.floor(
      (today.getTime() - lastPayday.getTime()) / millisecondsPerDay
    );
    ...
    const daysInFullPeriod = Math.ceil(
      (nextPayday.getTime() - lastPayday.getTime()) / millisecondsPerDay
    );
    ...
    const actualDailyBudget =
      daysForCalculation > 0 ? remainingBudget / daysForCalculation : remainingBudget;

    // Budget quotidien ajusté : basé uniquement sur les jours restants jusqu'à la paie
    // C'est le budget réaliste pour les dépenses quotidiennes
  ...
  getPaydayMessage(paydayInfo: PaydayInfo, remainingBudget: number): string {
    ...
    const { monthProgressPercent, remainingDaysInMonth, daysUntilPayday, actualDailyBudget } =
      paydayInfo;
